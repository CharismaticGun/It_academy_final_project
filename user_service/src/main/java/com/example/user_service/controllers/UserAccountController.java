...
public class UserAccountController implements IAccountController {
    ...
    @Override
    public ResponseEntity<UserRegistration> userReg(UserRegistration user) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.userRegistration(user));
    }
